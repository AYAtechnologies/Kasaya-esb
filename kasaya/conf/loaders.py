...
class CombinedConfig(object):
    ...
    def load_config(self, filename, globmode, optional=False):
        try:
    ...
    def options(self, section):
        known=set()
        for cnf, gmode in self.confs:
            # global section or services own config
            try:
    ...
    def items(self, section):
        for opt in self.options(section):
            print "key:",opt
            print "    ",
            yield opt, self.__getitem__(opt)



def load_config_file(filename, section, optional, set_value):
    """
